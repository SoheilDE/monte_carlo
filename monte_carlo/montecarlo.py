...
class MonteCarlo:
    ...
    def opt_policy(self, params_policy):
        ret = np.zeros([self.num_observ, self.num_action, 1])
        ret = ret.tolist()
        ...
        for num in range(num_epi):
            ...
            for (state, action) in results_list:
                G = 0.0
                if (state, action) in epi_list:
                    j += 1
                    pass
                else:
                    for i in range(len(episod_reward) - 1, j - 1, -1):
                        G = gamma * G + episod_reward[i]
                        epi_list.append((state, action))

                    ret[state][action].append(G)
                    j += 1
                q[state, action] = sum(ret[state][action]) / (len(ret[state][action]) - 1)
            optimal_policy = []
